...
export default function AgentLayout() {
  ...
  const isActive = (href: string) => location.pathname === href || (href !== '/agent/dashboard' && location.pathname.startsWith(href))
    ...
    <div className="min-h-screen flex bg-eventra-slate-50">
      ...
      >
        <div className="flex h-16 items-center justify-between px-6 border-b border-eventra-slate-200">
          <Link to="/agent/dashboard" className="flex items-center gap-2" aria-label="EventraOS Agent">
            ...
            <span className="font-display font-bold text-heading-md text-eventra-navy-900">EventraOS Agent</span>
          </Link>
          <button
            onClick={() => setSidebarOpen(false)}
        ...
        <nav className="flex-1 p-4 space-y-1 overflow-y-auto" aria-label="Agent menu">
          {agentNav.map((item) => (
            <NavLink
              key={item.name}
              to={item.href}
              className={({ isActive }) =>
                cn(
                  ...
                  isActive
                    ? 'bg-eventra-blue-600 text-white shadow-card'
                    : 'text-eventra-slate-600 hover:bg-eventra-blue-50 hover:text-eventra-blue-700'
                )
              }
              onClick={() => setSidebarOpen(false)}
            >
        ...
        <div className="p-4 border-t border-eventra-slate-200">
          <div className="flex items-center gap-3 px-3 py-2">
            {user?.avatar ? (
              ...
              <div className="w-9 h-9 rounded-full bg-eventra-blue-100 flex items-center justify-center">
                <span className="text-eventra-blue-700 font-medium">{user?.name?.charAt(0).toUpperCase()}</span>
              </div>
            )}
            ...
            <span className="badge badge-primary text-body-xs">Agent</span>
          </div>
        </div>
      </motion.aside>
          ...
          <div className="flex h-16 items-center justify-between px-6 gap-4">
            ...
            <div className="flex-1 max-w-xl">
              <h1 className="text-heading-lg font-display font-semibold text-eventra-navy-900 truncate">
                {agentNav.find((item) => isActive(item.href))?.name || 'Dashboard'}
              </h1>
            </div>
                  ...
                  {userMenuOpen && (
                    ...
                    >
                      <div className="px-4 py-3 border-b border-eventra-slate-200">
                        ...
                        <span className="badge badge-primary mt-1.5">Travel Agent</span>
                      </div>
