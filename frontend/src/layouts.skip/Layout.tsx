...
const navigation = [
  { name: 'Home', href: '/', icon: Home, roles: ['customer', 'agent', 'admin'] },
  { name: 'Search', href: '/search', icon: Search, roles: ['customer', 'agent', 'admin'] },
  { name: 'Hotels', href: '/hotels', icon: Building2, roles: ['customer', 'agent', 'admin'] },
  { name: 'Flights', href: '/flights', icon: Plane, roles: ['customer', 'agent', 'admin'] },
  { name: 'Trains', href: '/trains', icon: Train, roles: ['customer', 'agent', 'admin'] },
  { name: 'Buses', href: '/buses', icon: Bus, roles: ['customer', 'agent', 'admin'] },
  { name: 'Venues', href: '/venues', icon: MapPin, roles: ['customer', 'agent', 'admin'] },
  { name: 'Cars', href: '/cars', icon: Car, roles: ['customer', 'agent', 'admin'] },
  { name: 'Activities', href: '/activities', icon: Ship, roles: ['customer', 'agent', 'admin'] },
  { name: 'Transfers', href: '/transfers', icon: MapPin, roles: ['customer', 'agent', 'admin'] },
  { name: 'Packages', href: '/packages', icon: Briefcase, roles: ['customer', 'agent', 'admin'] },
]

const customerNav = [
  { name: 'Dashboard', href: '/customer/dashboard', icon: LayoutDashboard },
  { name: 'My Trips', href: '/customer/trips', icon: Suitcase },
...
const agentNav = [
  ...
  { name: 'Booking Workspace', href: '/agent/booking-workspace', icon: Briefcase },
  { name: 'Quotes', href: '/agent/quotes', icon: FileText },
  { name: 'Bookings', href: '/agent/bookings', icon: ClipboardList },
  { name: 'Commissions', href: '/agent/commissions', icon: Target },
  { name: 'Tasks', href: '/agent/tasks', icon: CheckSquare },
  { name: 'Support', href: '/agent/support', icon: MessageSquare },
]
...
import { FileText, UserCheck, CheckSquare, RotateCcw } from 'lucide-react'
      ...
      <header className="sticky top-0 z-40 bg-white/95 backdrop-blur-xl border-b border-eventra-slate-200">
        <nav className="section-container" aria-label="Main navigation">
          <div className="flex h-16 items-center justify-between gap-4">
            ...
            {/* Desktop Navigation */}
            <div className="hidden md:flex items-center gap-1 flex-1 max-w-4xl mx-8">
              {navigation.map((item) => (
                hasRole(item.roles[0] as 'customer' | 'agent' | 'admin') && (
                  <NavLink
                    key={item.name}
                    to={item.href}
                    className={({ isActive }) =>
            ...
            <div className="flex items-center gap-2 shrink-0">
              ...
              {cartCount > 0 && (
                ...
                >
                  <Suitcase className="w-5 h-5" aria-hidden="true" />
                    ...
                    {userMenuOpen && (
                      ...
                      >
                        <div className="px-4 py-3 border-b border-eventra-slate-200">
                          ...
                          <span className={`badge mt-1.5 ${user?.role === 'admin' ? 'badge-danger' : user?.role === 'agent' ? 'badge-primary' : 'badge-success'}`}>
                            {user?.role}
                          </span>
                        </div>
                        ...
                        {hasRole('customer') && (
                          <>
                            ...
                            <Link to="/customer/trips" className="dropdown-item" role="menuitem">
                              <Suitcase className="w-4 h-4" aria-hidden="true" />
                              My Trips
                            </Link>
                            ...
                            <Link to="/customer/loyalty" className="dropdown-item" role="menuitem">
                        ...
                        {hasRole('agent') && (
                          <>
                            <Link to="/agent/dashboard" className="dropdown-item" role="menuitem">
                              <LayoutDashboard className="w-4 h-4" aria-hidden="true" />
                              Agent Dashboard
                            </Link>
                            <Link to="/agent/booking-workspace" className="dropdown-item" role="menuitem">
                              <Briefcase className="w-4 h-4" aria-hidden="true" />
                              Booking Workspace
                            </Link>
                            <Link to="/agent/customers" className="dropdown-item" role="menuitem">
                              <Users className="w-4 h-4" aria-hidden="true" />
                              Customers
                            </Link>
                            <Link to="/agent/commissions" className="dropdown-item" role="menuitem">
                        ...
                        {hasRole('admin') && (
                          <>
                            <Link to="/admin/dashboard" className="dropdown-item" role="menuitem">
                              <LayoutDashboard className="w-4 h-4" aria-hidden="true" />
                              Admin Dashboard
                            </Link>
                            <div className="dropdown-divider" role="separator" />
        ...
        <AnimatePresence>
          {mobileMenuOpen && (
            <motion.div
              ...
              className="md:hidden border-t border-eventra-slate-200 bg-white animate-slide-down"
            >
...
function HotelSearchForm({ onClose }: { onClose: () => void }) {
  return (
    <form onSubmit={(e) => { e.preventDefault(); onClose(); window.location.href = '/hotels'; }} className="space-y-4">
...
function FlightSearchForm({ onClose }: { onClose: () => void }) {
  return (
    <form onSubmit={(e) => { e.preventDefault(); onClose(); window.location.href = '/flights'; }} className="space-y-4">
      <div className="grid gap-4 sm:grid-cols-2">
        <div>
...
function VenueSearchForm({ onClose }: { onClose: () => void }) {
  return (
    <form onSubmit={(e) => { e.preventDefault(); onClose(); window.location.href = '/venues'; }} className="space-y-4">
      <div className="grid gap-4 sm:grid-cols-2">
        <div>
