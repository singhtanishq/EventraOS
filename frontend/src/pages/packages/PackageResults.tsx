...
import { ChevronLeft, ChevronRight, Filter, SlidersHorizontal, MapPin, Star, Tag, X, Loader2, Plane, Building2, Car, Sparkles, MapPin as MapPinIcon, Shield, CheckCircle2, Calendar, Users, Package, Clock, Zap } from 'lucide-react'
...
export function PackageResults() {
  ...
  const { data, isLoading, isError, error, refetch } = useQuery({
    ...
    queryFn: async () => {
      const response = await api.get('/search/packages', { params: queryParams })
      return response.data
    },
    placeholderData: (previousData) => previousData,
  })

  const packages = data?.data?.results || []
            ...
            <div className="flex items-center gap-4">
              ...
              {searchParams.get('start_date') && searchParams.get('end_date') && (
                <span className="badge badge-neutral">
                  ...
                  {formatDate(searchParams.get('start_date')!)} - {formatDate(searchParams.get('end_date')!)}
                </span>
              )}
                      ...
                      >
                        {packages.map((pkg) => (
                          <PackageResultCard
                            ...
                            pkg={pkg}
                            onSelect={() => {
                              navigate(`/packages/${pkg.provider_item_id}`, {
                                state: { 
        ...
        <div className="mt-auto pt-4 border-t border-eventra-slate-200 flex items-center justify-between">
          <div>
            {metadata.pricing_tiers.length > 1 ? (
              <>
                <p className="price-lg text-eventra-navy-900">{formatCurrency(Math.min(...metadata.pricing_tiers.map(t => t.price)), pkg.pricing.currency)}</p>
