...
import { ChevronLeft, ChevronRight, SlidersHorizontal, Plane, Calendar, Users, Luggage, Car, Zap, Shield, Bus } from 'lucide-react'
...
export function TransferResults() {
  ...
  // date is required by the API validator - default to today
  const transferDate = filters.date || searchParams.get('date') || todayISO
  // time must be H:i if provided - only send it when it matches the expected format
  const transferTime = filters.time || searchParams.get('time') || ''
  const validTime = /^\d{2}:\d{2}$/.test(transferTime) ? transferTime : undefined
  ...
  const { data, isLoading, isError, error, refetch } = useQuery({
    ...
    queryFn: async () => {
      const body = await api.get<any>('/search/transfers', { params: queryParams })
      return body
    },
    placeholderData: (previousData) => previousData,
  })

  const transfers: TransferResult[] = data?.data?.results ?? []
  ...
  const handleSelect = (transfer: TransferResult) => {
    addItem({
      type: 'transfer',
      serviceId: transfer.provider_item_id || transfer.id,
      providerId: transfer.provider_code,
      name: `${transfer.metadata.operator_name} · ${transfer.metadata.vehicle_type_name}`,
      image: transfer.images[0],
      dates: { start: transferDate },
      passengers: parseInt(searchParams.get('passengers') || '1') || 1,
      options: [],
      pricing: {
        basePrice: transfer.pricing.total ?? 0,
        taxes: 0,
        fees: 0,
        serviceFee: 0,
        discount: 0,
        optionsTotal: 0,
        total: transfer.pricing.total ?? 0,
        currency: transfer.pricing.currency || 'INR',
        breakdown: [],
      },
                      ...
                      >
                        {transfers.map((transfer) => (
                          <TransferResultCard
                            ...
                            onSelect={() => handleSelect(transfer)}
                          />
                        ))}
                      </motion.div>
                    </AnimatePresence>
