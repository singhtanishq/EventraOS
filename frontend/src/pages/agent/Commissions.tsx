...
export function Commissions() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['agent-commissions', search, filterStatus, filterType, sortBy, dateRange],
    queryFn: async () => {
      ...
      const response = await api.get('/agent/commissions', { params })
  ...
  const filteredCommissions = commissions.filter(c => {
    ...
    if (filterType !== 'all' && c.service_type !== filterType) return false
    return true
  })
  ...
  return (
    <div className="space-y-6 animate-in">
      ...
      {summary && (
        ...
        >
          <StatCard icon={<DollarSign className="w-6 h-6" />} label="Total Earned" value={formatCurrency(summary.total_earned || 0, 'INR')} color="eventra-green" />
          <StatCard icon={<Clock className="w-6 h-6" />} label="Pending" value={formatCurrency(summary.pending || 0, 'INR')} color="eventra-amber" />
          <StatCard icon={<CheckCircle2 className="w-6 h-6" />} label="Eligible" value={formatCurrency(summary.eligible || 0, 'INR')} color="eventra-blue" />
          <StatCard icon={<CheckCircle2 className="w-6 h-6" />} label="Approved" value={formatCurrency(summary.approved || 0, 'INR')} color="eventra-green" />
          <StatCard icon={<DollarSign className="w-6 h-6" />} label="Paid" value={formatCurrency(summary.paid || 0, 'INR')} color="eventra-green" />
        </motion.div>
      )}
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-eventra-slate-200">
                    ...
                    <th className="px-4 py-3 text-left text-body-xs font-semibold text-eventra-slate-500 uppercase tracking-wider">Service</th>
                ...
                <tbody>
                  {filteredCommissions.map((commission, index) => (
                    <tr key={commission.id} className="border-b border-eventra-slate-100 hover:bg-eventra-slate-50">
                      ...
                      <td className="px-4 py-3">
                        <span className="badge badge-neutral capitalize">{commission.service_type}</span>
                      </td>
          ...
          <tbody>
            {[...Array(5)].map((_, i) => (
              <tr key={i}>
                ...
                <td className="px-4 py-3"><div className="h-4 w-24 bg-eventra-slate-200 animate-pulse rounded" /></td>
                <td className="px-4 py-3"><div className="h-4 w-24 bg-eventra-slate-200 animate-pulse rounded" /></td>
                <td className="px-4 py-3"><div className="h-4 w-20 bg-eventra-slate-200 animate-pulse rounded" /></td>
                <td className="px-4 py-3"><div className="h-4 w-24 bg-eventra-slate-200 animate-pulse rounded" /></td>
                <td className="px-4 py-3"><div className="h-4 w-24 bg-eventra-slate-200 animate-pulse rounded" /></td>
                <td className="px-4 py-3"><div className="h-4 w-24 bg-eventra-slate-200 animate-pulse rounded" /></td>
                <td className="px-4 py-3"><div className="h-4 w-24 bg-eventra-slate-200 animate-pulse rounded" /></td>
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center py-16">
      ...
      <p className="text-eventra-slate-600 mb-6 max-w-md mx-auto">
        Complete bookings for your customers to start earning commissions.
      </p>
    </motion.div>
  )
}