...
export function TrainResults() {
  ...
  const { data, isLoading, isError, error, refetch } = useQuery({
    ...
    queryFn: async () => {
      const body = await api.get<any>('/search/trains', { params: queryParams })
      return body
    },
    placeholderData: (previousData) => previousData,
  })

  const trains: TrainResult[] = data?.data?.results ?? []
  ...
  const handleSelect = (train: TrainResult) => {
    addItem({
      type: 'train',
      serviceId: train.provider_item_id || train.id,
      providerId: train.provider_code,
      name: `${train.metadata.train_number} · ${train.metadata.train_name}`,
      image: train.images[0],
      dates: { start: journeyDate },
      passengers: parseInt(searchParams.get('passengers') || '1') || 1,
      options: [],
      pricing: {
        basePrice: train.pricing.total ?? 0,
        taxes: 0,
        fees: 0,
        serviceFee: 0,
        discount: 0,
        optionsTotal: 0,
        total: train.pricing.total ?? 0,
        currency: train.pricing.currency || 'INR',
        breakdown: [],
      },
